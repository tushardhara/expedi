	...
	if($purchase_code !== ''){
		// API Key - m738ri6qkkylw8cttgaja2hiz1ax3kjv
		$url = 'http://marketplace.envato.com/api/edge/brainstormforce/m738ri6qkkylw8cttgaja2hiz1ax3kjv/verify-purchase:'.$purchase_code.'.json';
		$json = wp_remote_get($url);
		$result = json_decode($json['body'], true);
		if(isset($result['verify-purchase']['buyer']) && $result['verify-purchase']['buyer'] == $envato_username){
			$verify_label = '<span class="verification" style="top: -41px; position: absolute; right: 110px; padding: 1px 5px; background: green;  color: #fff; border-radius: 2px; cursor: context-menu;"> Verified </span>';
			...
			if($status == 'Activated' ){
				...
				$process = "reactivate";
				$activate_lable = '<span class="activation" style="top: -41px; position: absolute; right: 20px;  padding: 1px 5px; background: red;  color: #fff; border-radius: 2px; cursor: context-menu;">Deactivated</span>';
			} else {
				$process = "activate";
