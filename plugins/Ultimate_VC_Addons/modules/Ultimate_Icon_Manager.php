...
if(!class_exists('AIO_Icon_Manager'))
{
	class AIO_Icon_Manager
	{
		...
		function checkboxes_param($settings, $value){
			...
			if(is_array($options) && !empty($options)){
				foreach($options as $key => $opts){
					...
					$output .= '<div class="onoffswitch">
							<input type="checkbox" name="'.$param_name.'[]" value="'.$key.'" '.$dependency.' class="onoffswitch-checkbox chk-switch-'.$un.'" id="switch'.$uid.'" '.$checked.'>
			...
			$output .= '<input type="hidden" id="chk-switch-'.$un.'" class="wpb_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" name="' . $param_name . '" value="'.$value.'" '.$dependency.' />
';
			$output .= '<script type="text/javascript">
				jQuery(".chk-switch-'.$un.'").change(function(){
					 var val = "";
					 var l = jQuery(".chk-switch:checked").length;
					 var c = 1;
		...
		function animator_param($settings, $value){
			...
			$type = isset($settings['type']) ? $settings['type'] : '';
			$class = isset($settings['class']) ? $settings['class'] : '';
			$json = file_get_contents(plugins_url('../assets/js/animate-config.json',__FILE__));
