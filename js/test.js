(function($) {
    ...
    var topt;
    window.peGmapStyle = [{stylers: [{saturation: -100}]}, {featureType: "road",elementType: "geometry",stylers: [{lightness: 100}, {visibility: "simplified"}]}, {featureType: "road",elementType: "labels",stylers: [{visibility: "off"}]}];
    function imgfilter() {
    ...
    pixelentity.classes.Controller = function() {
        ...
        function widgets(el, controller) {
            ...
            el.find(".peSlider.peVolo").attr({"data-controls-arrows": "edges-buttons","data-controls-bullets": "enabled","data-icon-font": "enabled"});
            el.find(".carouselBox").attr({"data-height": "0,0"});
            el.find("a.peVideo").attr({"data-autoplay": "disabled"});
            el.find(".peBackground").attr({"data-mobile": mobile ? "true" : ""});
            ...
            if (bgyt.length > 0) {
                if (!$.fn.mb_YTPlayer || mobile || ie8 || window.peBgVideo === false) {
                    if (bgyt.attr("data-fallback")) {
                        bgyt.css({"background-image": "url(%0)".format(bgyt.attr("data-fallback"))}).addClass("pe-active")
                    }
                } else {
                    var def;
                    try {
        ...
        $.extend(this, {widgets: widgets,start: start})
    }
}(jQuery));